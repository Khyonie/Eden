...
import com.yukiemeralis.blogspot.eden.auth.old.SecurePlayerAccount.AccountType;
...
public class Permissions 
{
    ...
    {
        if ((!Boolean.valueOf(Eden.getModuleManager().getEnabledModuleByName("EdenAuth").getConfig().get("allow_console_logins")) && username.equals("console")) ||
    ...
    private static void onFailedLogin(CommandSender user, String username)
    {
        if (max_failed_login_attempts == -1)
            max_failed_login_attempts = Integer.parseInt(Eden.getModuleManager().getEnabledModuleByName("EdenAuth").getConfig().get("max_failed_login_attempts"));

        if (max_failed_login_attempts != Integer.parseInt(Eden.getModuleManager().getEnabledModuleByName("EdenAuth").getConfig().get("max_failed_login_attempts")))
            max_failed_login_attempts = Integer.parseInt(Eden.getModuleManager().getEnabledModuleByName("EdenAuth").getConfig().get("max_failed_login_attempts"));
        ...
        if (!login_attempts.containsKey(username))
        {
            ...
            int lockoutTime = 300;
            try {
                lockoutTime = Integer.parseInt(Eden.getModuleManager().getEnabledModuleByName("EdenAuth").getConfig().get("account_lockout_time"));
