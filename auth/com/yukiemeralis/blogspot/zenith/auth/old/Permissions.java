...
public class Permissions 
{
    ...
    public static synchronized boolean registerAccount(SecurePlayerAccount account)
    {
        ...
        JsonUtils.toJsonFile("./plugins/Zenith/user-accounts/" + account.getUsername() + ".json", account);
        accounts.put(account.getUsername(), account);
        return true;
    }
            ...
            public void run()
            {
                ...
                if (account.getAccountType().getRank() > 0)
                {
                    ...
                    for (CommandSender s : logged_in_users.keySet())
                    {
                        ...
                        {
                            PrintUtils.sendMessage(sender, "Player " + ((Player) sender).getName() + " has made an account request. Run \"/zen requests view\" for more information.");
                        }
                    }
    ...
    private static void onFailedLogin(CommandSender user, String username)
    {
        if (max_failed_login_attempts == -1)
            max_failed_login_attempts = Integer.parseInt(Zenith.getModuleManager().getEnabledModuleByName("ZenithAuth").getConfig().get("max_failed_login_attempts"));

        if (max_failed_login_attempts != Integer.parseInt(Zenith.getModuleManager().getEnabledModuleByName("ZenithAuth").getConfig().get("max_failed_login_attempts")))
            max_failed_login_attempts = Integer.parseInt(Zenith.getModuleManager().getEnabledModuleByName("ZenithAuth").getConfig().get("max_failed_login_attempts"));
        ...
        if (!login_attempts.containsKey(username))
        {
            ...
            int lockoutTime = 300;
            try {
                lockoutTime = Integer.parseInt(Zenith.getModuleManager().getEnabledModuleByName("ZenithAuth").getConfig().get("account_lockout_time"));
            ...
            }.runTaskLater(Zenith.getInstance(), lockoutTime*20);
