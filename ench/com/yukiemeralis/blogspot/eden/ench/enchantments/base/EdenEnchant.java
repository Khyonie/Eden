package com.yukiemeralis.blogspot.eden.ench.enchantments.base;
...
public class EdenEnchant 
{
    ...
    @SafeVarargs
    public EdenEnchant(String name, String description, Class<? extends Event> applicableEvent, CompatibleItem... compatibleWith)
    {
        this.name = name;
    ...
    public static List<EdenEnchant> getEnchantments(ItemStack item, Class<? extends Event> event)
    {
    ...
    public static Result<Boolean, String> apply(ItemStack item, EdenEnchant enchant, int level)
    {
        ...
        if (!ItemUtils.hasNamespacedKey(item, "hasEdenEnchant"))
            ItemUtils.saveToNamespacedKey(item, "hasEdenEnchant", "0"); // Lore pointer
        ...
        if (!ItemUtils.hasNamespacedKey(item, enchant.getEvent().getSimpleName()))
        {
            ...
            List<EdenEnchant> enchants = csvToEnchList(ItemUtils.readFromNamespacedKey(item, enchant.getEvent().getSimpleName()));
            ...
            enchants.removeIf(e -> {
                ...
                {
                    ItemUtils.saveToNamespacedKey(item, "hasEdenEnchant", (Integer.parseInt(ItemUtils.readFromNamespacedKey(item, "hasEdenEnchant")) - 1) + "");
        ...
        // Lore settings
        enchant.setLoreLine(Integer.parseInt(ItemUtils.readFromNamespacedKey(item, "hasEdenEnchant")));
    ...
    private static String enchListToCsv(List<EdenEnchant> enchants)
    {
    ...
    private static List<EdenEnchant> csvToEnchList(String csv)
    {
        List<EdenEnchant> enchBuffer = new ArrayList<>();
        ...
        data.forEach((clazz, level) -> {
            EdenEnchant ench = null;

            Result<EdenEnchant, String> result = getEnchantByClassname(clazz);
            switch (result.getState())
            {
                case OK:
                    try {
                        ench = (EdenEnchant) result.unwrap();
    ...
    @SuppressWarnings("unchecked") // Everything in the given package will extend EdenEnchant
    public static Result<EdenEnchant, String> getEnchantByClassname(String className)
    {
        Result<EdenEnchant, String> result = new Result<>(EdenEnchant.class, String.class);

        EdenEnchant enchant;
        try {
            Class<? extends EdenEnchant> enchClass = (Class<? extends EdenEnchant>) Class.forName("com.yukiemeralis.blogspot.eden.ench.enchantments." + className);
