...
public class PlayerReachLocationTrigger extends EventTrigger<PlayerMoveEvent>
{
    ...
    public PlayerReachLocationTrigger(boolean synchronizeAttendees, int step, Location location, double radius, EdenEvent... events)
    {
    ...
    public boolean shouldTrigger(PlayerMoveEvent event)
    {
        if (!event.getPlayer().getWorld().equals(location.getWorld())) // Can't compare two different worlds
            return false;
        if (!this.getAssociatedEvents().get(0).getScript().equals(EdenEvents.getEventAttendedBy(event.getPlayer()))) // Ensure the player is attending this script
            return false;
